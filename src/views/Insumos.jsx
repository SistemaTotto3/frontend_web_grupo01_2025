...
const Insumos = () => {
  ...
  const generarPDFInsumos = () => {
    ...
    doc.setFontSize(28);
    doc.text("Lista de Insumos", doc.internal.pageSize.getWidth() / 2, 20, {
      align: "center",
    });
    ...
    const filas = insumosFiltrados.map((insumo) => [
      insumo.id_insumo,
      new Date(insumo.fecha_insumo).toLocaleDateString("es-NI", {
  ...
  const obtenerDetallesInsumo = async (id_insumo) => {
    try {
      ...
      const filtrados = todos.filter(
        (d) => d.id_insumo === parseInt(id_insumo)
      );
      ...
      const detalles = await Promise.all(
        filtrados.map(async (d) => ({
          ...d,
          nombre_insumo:
            d.nombre_insumo || (await obtenerNombreProducto(d.id_producto)), // Priorizar nombre_insumo, fallback a producto
        }))
      );
  ...
  const agregarInsumo = async () => {
    ...
    const total = detallesNuevos.reduce(
      (sum, d) => sum + d.cantidad_insumo * d.precio_insumo,
      0
    );

    try {
      const insumoResp = await fetch(
        "http://localhost:3000/api/registrarinsumo",
        {
          method: "POST",
  ...
  const actualizarInsumo = async () => {
    ...
    try {
      await fetch(
        `http://localhost:3000/api/actualizarInsumoPatch/${insumoAEditar.id_insumo}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      const actuales = todos.filter(
        (d) => d.id_insumo === insumoAEditar.id_insumo
      );
      for (const d of actuales) {
        await fetch(
          `http://localhost:3000/api/eliminardetalle_insumo/${d.id_detalle_insumo}`,
          { method: "DELETE" }
        );
      }
  ...
  const eliminarInsumo = async () => {
    try {
      await fetch(
        `http://localhost:3000/api/eliminarinsumo/${insumoAEliminar.id_insumo}`,
        { method: "DELETE" }
      );
      await obtenerInsumos();
